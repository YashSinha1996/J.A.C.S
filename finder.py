...
def evaluate(corr,inp,folder,fault,limit,time,is_file,file_check=None,all_regex=False):
    ...
    for to_test in files:
        if is_file:
            how_ss=tester.from_file(corr=corr,inp=inp,folder=folder,file=os.path.join(folder, to_test),fault=fault,limit=limit,time=timeout)
        else:
            how_ss=tester.regex_test(corr=corr,inp=inp,folder=folder,file=os.path.join(folder, to_test),fault=fault,limit=limit,time=timeout)
        how=how_ss[0]
        if how==0:
...
def crawlDir(corr,inp,folder,fault,limit,time,is_file,level,file_check=None,multi_check=[],all_regex=False):
    if level==0:
        evaluate(corr=corr,inp=inp,folder=folder,fault=fault,limit=limit,time=timeout,is_file=is_file,file_check=file_check,all_regex=all_regex)
        return
    ...
    for sub_dir in sub_dir_list:
        crawlDir(corr=corr,inp=inp,folder=os.path.join(folder, sub_dir),fault=fault,limit=limit,time=timeout,is_file=is_file,level=level-1,file_check=file_check,multi_check=multi_check,all_regex=all_regex)
